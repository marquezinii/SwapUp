(() => {
  ...
  const defaultSettings = {language:'pt-BR',startView:'month',autostart:true,closeToTray:true,confirmDelete:true,theme:'light',accent:'#6757e8',compact:false,reduceMotion:false,fontScale:1,highContrast:false,reduceTransparency:false,showInsight:true,weekStart:0,timeFormat:'24',duration:60,defaultReminder:15,defaultCategory:'pessoal',notifications:true,notifyLocation:true,calendarSystem:'gregory'};
  ...
  function load(){
    try { const v=JSON.parse(localStorage.getItem('swapup-schedule-data')||localStorage.getItem('swapup-data')); if(v?.version){const settings={...defaultSettings,...(v.settings||{})};if(v.version===1&&!v.settings)settings.theme='light';return {...v,version:3,filters:v.filters||{},settings,profile:{...defaultProfile,...(v.profile||{})}};} } catch{}
  ...
  function save(){ localStorage.setItem('swapup-schedule-data',JSON.stringify(state));localStorage.removeItem('swapup-data'); }
  ...
  function renderBrand(){document.title=I18n.brand;$$('[data-brand]').forEach(element=>element.textContent=I18n.brand);}
  ...
  function applyPreferences(){
    ...
    post({type:'setCloseBehavior',closeToTray:!!state.settings.closeToTray});
    renderProfileAvatars();
    post({type:'setLanguage',strings:{brand:I18n.brand,open:I18n.t('Abrir SwapUp! Agenda ©'),newEvent:I18n.t('Novo evento'),exit:I18n.t('Sair'),windowTitle:I18n.t('Seu calendário'),trayTitle:I18n.t('SwapUp! Agenda © continua ativo'),trayBody:I18n.t('Seus lembretes ficam funcionando na bandeja. Clique duas vezes no ícone para abrir.'),duplicate:I18n.t('O SwapUp! Agenda © já está aberto na bandeja do sistema.')}});
  }
  ...
  function renderTimeline(mode){
    ...
    const scroll=target.querySelector('.timeline-scroll'),header=target.querySelector('.timeline-header'),allDayGrid=target.querySelector('.all-day-grid'),now=new Date(),preferred=Math.max(startHour,Math.min(endHour-1,now.getHours()))-startHour;if(scroll){const syncTimeline=()=>{const offset=-scroll.scrollLeft;header.style.transform=`translateX(${offset}px)`;allDayGrid.style.transform=`translateX(${offset}px)`;};scroll.addEventListener('scroll',syncTimeline,{passive:true});scroll.scrollTop=Math.max(0,preferred*64-100);syncTimeline();}
  }
  ...
  function exportBackup(){post({type:'export',data:state,strings:{title:I18n.t('Salvar backup do SwapUp! Agenda ©'),backup:I18n.t('Backup SwapUp! Agenda ©'),json:I18n.t('Arquivo JSON')}});}
  function importBackup(){post({type:'import',strings:{title:I18n.t('Restaurar backup do SwapUp! Agenda ©'),backup:I18n.t('Backup SwapUp! Agenda ©')}});}
  ...
  function fillSettings(){
    ...
    $('#settingStartView').value=state.settings.startView;$('#settingAutostart').checked=!!state.settings.autostart;$('#settingCloseToTray').checked=!!state.settings.closeToTray;$('#settingConfirmDelete').checked=!!state.settings.confirmDelete;
  ...
  function checkReminders(){
    ...
    const now=new Date(), notified=JSON.parse(localStorage.getItem('swapup-schedule-notified')||localStorage.getItem('swapup-notified')||'{}');
    ...
    const cutoff=Date.now()-2592000000;for(const k of Object.keys(notified))if(notified[k]<cutoff)delete notified[k];localStorage.setItem('swapup-schedule-notified',JSON.stringify(notified));localStorage.removeItem('swapup-notified');
  }
  window.SwapUpSchedule={openNewEvent:()=>openNew(iso(new Date())),onBugReportResult:(status,message)=>setBugStatus(message,status==='sent'||status==='saved'),importData:(json)=>{try{const data=JSON.parse(json);if(!Array.isArray(data.events)||!Array.isArray(data.categories))throw 0;const importedSettings={...defaultSettings,...(data.settings||{})};if(data.version===1&&!data.settings)importedSettings.theme='light';state={...data,version:3,filters:data.filters||{},settings:importedSettings,profile:{...defaultProfile,...(data.profile||{})}};save();applyPreferences();switchView(state.settings.startView);toast('Backup restaurado com sucesso.');}catch{toast('Esse arquivo não é um backup válido do SwapUp! Agenda ©.');}}};
